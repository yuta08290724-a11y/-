...
三層でネタを収集してください。**各リクエストは1つずつ順番に実行すること（並行実行禁止）。**

**一次：Obsidian ネタ帳**
- `mcp__obsidian__search-vault` でキーワード「ネタ」「投稿候補」「メモ」をそれぞれ検索する
- ヒットしたノートは `mcp__obsidian__read-note` で本文を読み込む
- 今日使えそうなネタを抽出してリストアップする
- 検索結果が空の場合は「結果なし」と表示してスキップ
- あわせて `mcp__a55b48d7-98ec-42cc-a0eb-c277bba6036c__recall` でキーワード「ネタ」「投稿候補」「メモ」を検索し、Obsidianと合わせてリストアップする

**二次：ベンチマークアカウント監視（毎日固定）**
- scrape_social（twitter / user-tweets）で以下4アカウントの最新投稿を取得（各10件）、1アカウントずつ順番に実行する
  1. `rukamineral8`
  2. `bihadaoji_40`
  3. `kepiko_choukatu`
  4. `suinail`
- 取得後、以下を抽出してリストアップする
  - **フック**：冒頭1行の型（問いかけ / 否定形 / 数字 / あるある 等）
  - **構成**：投稿の流れ（問題提起→解決 / リスト形式 / ストーリー 等）
  - **言い回し**：バズりやすそうな表現・ワード
  - **テーマ**：何について書いているか
- 各アカウントの検索結果が空の場合は「結果なし」と表示してスキップ

**二次（後処理）：勝ちパターンの蓄積**
- 二次で抽出したフック・構成・言い回しのうち、以下の基準を満たすものを `mcp__a55b48d7-98ec-42cc-a0eb-c277bba6036c__memory` でSuper Memoryに保存する
  - 基準：「トッティーのリカバリーコンセプトに転用できる」「フックが強い」「構成が再現しやすい」
  - 保存フォーマット：
    ```
    [勝ちパターン] $CURRENT_DATE
    アカウント：@[handle]
...
- `mcp__a55b48d7-98ec-42cc-a0eb-c277bba6036c__recall` で「バズ」「高エンゲージ」「勝ちパターン」を検索
- 再現できる要素を抽出
- 検索結果が空の場合は「結果なし」と表示してスキップ
...
- Step 1 二次で抽出したベンチマークの「フック・構成・言い回し」を参考にすること。ただしパクリではなく、トッティーのコンセプトに合わせて昇華させること。
- **【絶対禁止】No.1〜No.5 すべての投稿本文に絵文字を一切使わないこと。ハッシュタグ・リカバリーメッセージ含め全行絵文字禁止。**

各投稿フォーマット（No.1〜No.4のみ。No.5はこのフォーマットを使わない）：
```
【投稿 No.X】
本文（140字以内、絵文字なし、改行で読みやすく）
---
ハッシュタグ：#〇〇 #〇〇
...
- **No.5**：**リカバリーレシピ枠**（毎日必須・フック種別「学び」固定）
  ...
  **【No.5 必須ルール】**
  - **【重要】上の【投稿 No.X】フォーマットは使わない。必ず下記【No.5 投稿フォーマット】だけを使うこと。**
  - **絵文字禁止**：本文・材料・手順・リカバリーメッセージ、すべての行に絵文字を使わない
  - **フォーマット厳守**：必ず下記【No.5 投稿フォーマット】の構成で出力すること。自由文にしない
